...
void* verticals(void* arg)
{
    int tno = (int64_t)arg;
    //The matrix is splitted vertically for NTHREADS:
    for (int i = (N/NTHREADS)*tno; i < (N/NTHREADS)*(tno+1); i += mc)
    {
...
void multithreading(double *Xx, double *Yy, double *Zz, int N)
{
    //Initializing the globals to avoid passing:
    X = Xx; Y = Yy; Z = Zz;

    pthread_t pthreads[NTHREADS];
    for (int t = 0; t < NTHREADS; ++t)
    {
        pthread_create(&pthreads[t], NULL, verticals, (void*)(int64_t)t);
    }

    for (int t = 0; t < NTHREADS; ++t) pthread_join(pthreads[t], NULL);
    
    //Wait for each of the threads to complete before returning to the main;
}